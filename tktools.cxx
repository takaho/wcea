...
tktools::bio::fasta_sequence::fasta_sequence( const char* filename ) throw ( exception ) {
    int fsize = tktools::io::get_file_size(filename);
    FILE* fp;
    fp = fopen( filename, "r" );
    if ( fp == NULL ) {
        throw invalid_argument( string("cannot open " ) + filename);
    }
    char* buffer = new char[ fsize ];
    size_t size_read = fread( buffer, sizeof(char), fsize, fp );
    fclose(fp);
    if (size_read == 0) {
      delete[] buffer;
    ...
    string label;
    const char* ptr_seq = buffer;
    const char* ptr_end = buffer + fsize;
    bool header = false;
    int line_start = 0;
    int line_end = 0;
    for ( int i = 0; i < fsize; i++ ) {
        if (line_start == 0 &&  buffer[ i ] == '>' ) {
            //int start = i + 1;
            //int end = start;
            line_start = i + 1;
        } else if (buffer[i] < ' ') {
    ...
    }
    {
        for ( int i = line_start; i < line_end; i++ ) {
            if ( strncmp(buffer + i, "chromosome", 10) == 0 
                 || strncmp(buffer + i, "Chromosome", 10) == 0 ) {
                int name_start = i;
                int name_end = line_end;
                for ( int j = i + 10; j < line_end; j++ ) {
                    if (buffer[j] > ' ') {
                        for (int k = j + 1; k < line_end; k++ ) {
                            if (buffer[k] < '0') {
                                name_end = k;
                                break;
                            }
                ...
                char* lbuf = new char[name_end - name_start + 1];
                memcpy(lbuf, buffer + name_start, name_end - name_start);
                lbuf[name_end - name_start] = '\0';
                _name = lbuf;
                delete[] lbuf;
                header = true;
            }
        ...
        if (_name.size() == 0) {
            char *lbuf = new char[line_end - line_start + 1];
            memcpy(lbuf, buffer + line_start, line_end - line_start);
            lbuf[line_end - line_start] = '\0';
            _name = lbuf;
            delete[] lbuf;
            header = true;
        }
    ...
    //                 ptr_seq = buffer + j + 1;
    //                 break;
    //             } else if ( c != ' ' && flag_start == false ) {
    //                 start = j;
    //                 flag_start = true;
    //             }
    //         }
    //         char* lbuf = new char[ end - start + 1 ];
    //         memcpy( lbuf, buffer + start, end - start );
    //         lbuf[ end - start ] = '\0';
    ...
    // }
    if ( !header ) {
        throw invalid_argument("no header line");
    }
    char* ptr_write = buffer;
    int bases = 0;
    while ( ptr_seq < ptr_end ) {
        char c = *ptr_seq;
        if ( (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') ) {
            *ptr_write = c;
            bases ++;
...
tktools::bio::fasta_sequence* tktools::bio::load_fasta_sequence( const char* filename ) throw ( exception ) {
    try {
        return new tktools::bio::fasta_sequence(filename);
    } catch ( exception& e ) {
        cerr << __FILE__ << " " << __LINE__ << endl;
        throw;
    }
...
vector<tktools::bio::fasta_sequence*> tktools::bio::fasta_sequence::load_fasta_sequences(const char* filename) throw (exception) {
    try {
        vector<tktools::bio::fasta_sequence*> sequences;
        ...
        while (!fi.eof()) {
            ...
            if (line.c_str()[0] == '>') {
                if (seq.size() > 0) {
                    fasta_sequence* fasta = new fasta_sequence();
                    fasta->_name = title;
                    fasta->set_sequence((int)seq.size(), seq.c_str());
                    sequences.push_back(fasta);
                }
                title = line.substr(1, line.size());
                seq = "";
